function [AGI_names, network1, sign_network1] = GRN_inference(d_long_scn,AGI,ind_pos,reg,set_max_reg,change_t,change_t_up,change_t_down,n_genes,n_levels,autoregulation,time_lapse)

% autoregulation is 0 or 1. If it is zero, the function will include
% autoregulation of genes. If it is zero, then genes cannot regulate
% themselves.
...
for i = 1:2  % this loop will calculate everything once for the regulators, and separately, a second time for the corregulators
    for j = 1:n_genes
        ...
        if ind1(1,j,i) > 0  % if var i has at least one regulator
            for k = 1:ind1(1,j,i) % for each regulator
                % Subset of regulators (all time points except for the last, because I need to align it with the target gene):
                E = d_long_scn(ind3(k),1:end-(-i+2))'; %(-i+2) is just to avoid an "if". when i==1, I want E to go from 1:end-1, and t to go from 2:end, but when i==2, i want both from 1:end. This (-i+2) solves the problem.
                t = d_long_scn(j,1+(-i+2):end)'; % Target gene (all time points except for the first, because I need to align it with the regulators):
...
%                     Nne = E ~= t;
                ...
                if ind1(1,j,i) > 1 & set_max_reg > 1
                    for kk = 1:k-1
                        E = d_long_scn([ind3(k),ind3(kk)],1:end-(-i+2))';
                        ...
                        if ind1(1,j,i) > 2 & set_max_reg > 2
                            for kkk = 1:kk-1
                                E = d_long_scn([ind3(k),ind3(kk),ind3(kkk)],1:end-(-i+2))';
...
% this matrix will store the maximum of each conditional probability, which
% will indicate the final regulators of each gene.
% 1 means activator; -1 means repressor. Each column indicates the
% regulator of 1 gene. The third dimension is to store 1 mat for the
% regulators and another one for the corregulators.
...
for i = 1:n_genes
    ...
    s2 = sign(:,i,2);
    for j = 1:length(s1)  
        network1(abs(cell2mat(h1(j))),i) = abs(BDe(i,1)); % All the BDe have negative values, so I want to make it positive if it was an activator
        sign_network1(abs(cell2mat(h1(j))),i) = 1; 
        if cell2mat(s1(j)) < 0
            network1(abs(cell2mat(h1(j))),i) = -abs(BDe(i,1)); % Negative for a repressor
            sign_network1(abs(cell2mat(h1(j))),i) = -1; 
    ...
    for j = 1:length(s2)       
        network2(abs(cell2mat(h2(j))),i) = abs(BDe(i,2));
...
m2 = max(max(abs(network2)));
network1 = network1/m1;
network2 = network2/m2;
...
nonzero_ind = network1 > 0;
discard_ind = abs(network1) < mean(mean(abs(network1(nonzero_ind))))/5;
...
%my_AGI = {'Gene 1','Gene 2','Gene 3','Gene 4','Gene 5','Gene 6'};
my_AGI1 = my_AGI;

if autoregulation == 1 
    sc1 = find(diag(network1));
    network1 = network1-diag(diag(network1)); 
    n = size(network1,1);
    m = numel(sc1);
    network1(n+m,n+m)=0;
    network1(sub2ind([n+m,n+m],[sc1;(1:m)'+n],[(1:m)'+n;sc1]))=1;
    my_AGI1((1:m)+n) = {' '};
end
...
for i = 1:size(bg1.edges)
    if get(bg1.edges(i),'Weight') < 0
        % set(bg1.edges(i),'LineColor',[0.7 0.7 0.7]);
