function GENIST(genes_file,time_course_file,clustering_data_file,time_lapse,is_load_new_data,TF_file,symbol_file)
%%
 ...
 %% Load data %%
    ...
    elseif nargin == 4
        if (time_lapse ~= 0 && time_lapse ~= 1)
            error('time_lapse must be 0 or 1')
        end
        is_load_new_data = true;
    else
        if (time_lapse ~= 0 && time_lapse ~= 1)
    ...
    S = std(pf_clust_data,0,2); % compute the variance of each expression pattern
...
%% 
    ...
    if length(TF_in_genes) < 10
        n_clusters = 1;
        [IDX,~] = kmeans(clust_data_norm,n_clusters,'Distance','sqeuclidean','Replicates',5); %'sqeuclidean' is the default distance
    else
        eva = evalclusters(clust_data_norm,'kmeans','silhouette','KList',[floor(length(TF_in_genes)/10):floor(length(TF_in_genes)/5)]); % 100-50
    ...
    if n_clusters > 1
        for i = 1:n_clusters
            n_IDX(i) = sum(eva.OptimalY==i);
            genes_per_cluter(1:n_IDX(i),i) = genes(eva.OptimalY == i);
        end
    else
...
%% 
 ...
 %% Find the GRN for each cluster   
    ...
    for i = 1:n_clusters
        ...
        %plot
        for j = 1:length(my_symbols)
            idx = ismember(sym(:,1),my_symbols(j),'rows'); 
            new_name = sym(idx,2);
            if ~isempty(new_name)
        ...
        if n_IDX(i) > 1
            [d_long_scn,reg,set_max_reg,change_t,change_t_up,change_t_down,n_genes,n_levels] = GRN_preprocessing_TFs_to_genes(g_int_temp,autoregulation,my_clust);

            [gene_names, network, sign_network] = GRN_inference(d_long_scn,ID_T,ind_pos,reg,set_max_reg,change_t,change_t_up,change_t_down,n_genes,n_levels,autoregulation,time_lapse);
        else
            gene_names = my_clust;
            network = 0;
            sign_network = 0;
    ...
    if n_clusters > 1
        ...
        [d_long_scn,reg,set_max_reg,change_t,change_t_up,change_t_down,n_genes,n_levels] = GRN_preprocessing(g_int_temp,autoregulation);

        [inter_clust_AGI, inter_clust_net, inter_sign_network] = GRN_inference(d_long_scn,ID_T,hubs_pos,reg,set_max_reg,change_t,change_t_up,change_t_down,n_genes,n_levels,autoregulation,time_lapse); % find connections among cluster hubs
        ...
        n_connections = sum(sum(bin_big_net))
        ...
        if autoregulation == 1 
            sc1 = find(diag(final_network));
            final_network = final_network-diag(diag(final_network)); 
            n = size(final_network,1);
            m = numel(sc1);
            final_network(n+m,n+m)=0;
            final_network(sub2ind([n+m,n+m],[sc1;(1:m)'+n],[(1:m)'+n;sc1]))=1;
            gene_names((1:m)+n) = {' '};
        end
        ...
        if autoregulation == 1 
            for i = 1:numel(bg1.Nodes)
                if i>n
                    bg1.Nodes(i).Shape = 'circle';
                    set(bg1.Nodes(i),'Size',[1,1]);
                end
            end
        end
    ...
    matrix_to_cytoscape_table(gene_names,final_network, final_sign_network);
end